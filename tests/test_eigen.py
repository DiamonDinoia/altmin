...
class TestDerivatives(unittest.TestCase):
    def test_relu_derivative(self):
        ...
        #Will work out whether to use numpy or torch for the python side later
...
class TestUpdateFunctions(unittest.TestCase):
    ...
    def test_update_last_layer(self):
        ...
        fast_altmin.cf_update_last_layer_eigen(model_cpp, in_tensor.detach(), targets.detach(), n_iter, lr, momentum_dict_cpp, True )
        fast_altmin.cf_update_last_layer_eigen(model_cpp, in_tensor.detach(), targets.detach(), n_iter, lr, momentum_dict_cpp, False )
    ...
    def test_update_hidden_weights(self):
        ...
        momentum_dict_cpp = fast_altmin.store_momentums(model_cpp, init_vals = True)
        # Run twice to test parameters returned correctly after first execution
        fast_altmin.cf_update_hidden_weights_eigen(model_cpp, in_tensor.detach(), codes, 0,n_iter, lr, momentum_dict_cpp, True)
        fast_altmin.cf_update_hidden_weights_eigen(model_cpp, in_tensor.detach(), codes, 0,n_iter, lr, momentum_dict_cpp, False)
    ...
    def test_update_codes(self):
        ...
        for x in range(len(codes_cpp)):
            check_equal(codes_python[x], codes_cpp[x], 10e16)
