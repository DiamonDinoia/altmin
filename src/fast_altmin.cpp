...
class Layer{
    public:
        ...
        Layer(layer_type layer_type, int batch_size, int n, int m, Eigen::MatrixXd weight, Eigen::VectorXd bias , bool  has_codes, double lr){
            ...
            // the exponentially decaying average of the squared weight gradients
            ...
            //Used to initialised the moments of the gradient and then set to false
            init_vals = true;
            this->lr = lr;
        ...
        void adam(Eigen::Ref<Eigen::MatrixXd> grad_weight, Eigen::Ref<Eigen::VectorXd> grad_bias){
        ...
        void differentiate_layer(Eigen::Ref<Eigen::MatrixXd> inputs, bool code_derivative){
            switch(layer){
                case(Layer::layer_type::relu):
...
class NeuralNetwork{
    public:
        ...
        NeuralNetwork(loss_function loss_fn, int n_iter_codes, int n_iter_weights, int batch_size, int m, double lr_codes, double mu, double momentum){

            ////////// Model hyperparemters /////////////////////////////////////////////////////////////////////////////////////////////////////////
            this->n_iter_codes = n_iter_codes;
        ...
        void calc_matrix_for_derivative(Eigen::Ref<Eigen::MatrixXd> inputs, int idx, int end_idx){            
            if (idx < end_idx){
        ...
        void apply_chain_rule(bool code_derivative){
...
NB_MODULE(fast_altmin, m) {
    ...
    NeuralNetwork.def(nb::init<NeuralNetwork::loss_function,int, int, int, int, double, double, double>())
