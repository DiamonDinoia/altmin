...
from altmin.datasets import  load_dataset
from altmin.utils import get_devices, ddict
...
if __name__ == "__main__":
    ...
    model = get_mods(model, optimizer='SGD', optimizer_params={'lr': args.lr_weights},
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for batch_idx, (data, targets) in enumerate(train_loader):
            ...
            # (2) Update codes
            codes = cf_update_codes(codes, model, targets, 0)

            # (3) Update weights
            cf_update_last_layer(model[-1], codes[-1], targets, 0, 1)

            cf_update_hidden_weights(model, data, codes, 0,1)
