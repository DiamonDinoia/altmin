...
if args.model.lower() == 'feedforward' or args.model.lower() == 'binary':
    ...
    model = LeNet(num_input_channels=num_input_channels,
                  window_size=window_size, bias=True).to(device)
...
if __name__ == "__main__":
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for batch_idx, (data, targets) in enumerate(train_loader):
            ...
            # (2) Update codes
            codes = update_codes(codes, model, targets, criterion, mu,
                                 lambda_c=args.lambda_c, n_iter=args.n_iter_codes, lr=args.lr_codes)

            # (3) Update weights
            update_last_layer_(
                model[-1], codes[-1], targets, criterion, n_iter=args.n_iter_weights)

            update_hidden_weights_adam_(
                model, data, codes, lambda_w=args.lambda_w, n_iter=args.n_iter_weights)

            # Store all iterations of first epoch
            if epoch == 1 and args.log_first_epoch:
                SAV.perf.first_epoch += [test(model,
                                              data_loader=test_loader, label=" - Test")]
        ...
        # Print performances
        SAV.perf.tr += [test(model, data_loader=train_loader,
                             label="Training")]
    ...
    if args.postprocessing_steps > 0:
        ...
        for epoch in range(1, args.postprocessing_steps+1):
            for batch_idx, (data, targets) in enumerate(train_loader):
                data, targets = data.to(device), targets.to(device)
                post_processing_step(model, data, targets,
                                     criterion, args.lambda_w)
    ...
    # Print performances
    SAV.perf.tr_final = test(
        model, data_loader=train_loader, label="  Training set after post-processing")
    SAV.perf.te_final = test(
        model, data_loader=test_loader, label="  Test set after post-processing    ")
