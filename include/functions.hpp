...
ALTMIN_INLINE auto lin(const Eigen::MatrixXd& input,
                       const Eigen::MatrixXd& weight,
                       const Eigen::VectorXd& bias) noexcept {
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> res = (input * weight.transpose());
    res += bias;
...
ALTMIN_INLINE auto sigmoid(const nanobind::DRef<Eigen::MatrixXd>& input) noexcept {
    ...
    for (auto i = 0l; i < N; ++i) {
        for (auto j = 0l; j < M; ++j) { res(i, j) = 1.0 / (1.0 + std::exp(-input(i, j))); }
    }
    return res;
}
...
ALTMIN_INLINE void sigmoid_inplace(nanobind::DRef<Eigen::MatrixXd> input) noexcept {
    ...
    const auto M = input.cols();
    for (auto i = 0l; i < N; ++i) {
        for (auto j = 0l; j < M; ++j) { input(i, j) = 1.0 / (1.0 + std::exp(-input(i, j))); }
    }
}
    ...
    for (long i = 0; i < N; ++i) {
        ...
        for (long j = 0; j < M; ++j) {
            sum += (targets.row(i)(j) * std::log(predictions.row(i)(j))) +
                   ((1.0 - targets.row(i)(j)) * std::log(1.0 - predictions.row(i)(j)));
        }
...
ALTMIN_INLINE void log_softmax(nanobind::DRef<Eigen::MatrixXd> input) noexcept {
    const auto N = input.rows();
    const auto M = input.cols();
    // TODO: Probably can be done using eigen instead of std::vector and various
    // loops
    std::vector<double> exps(M);
    for (long i = 0; i < N; ++i) {
        for (long j = 0; j < M; ++j) { exps[j] = std::exp(input(i, j)); }
        double sum_exps = 0.0;
        for (const auto exp : exps) { sum_exps += exp; }
        const auto inv_sum_exps = 1.0 / sum_exps;
        for (long j = 0; j < M; ++j) { input(i, j) = std::log(exps[j] * inv_sum_exps); }
    }
}
...
ALTMIN_INLINE void softmax(nanobind::DRef<Eigen::MatrixXd>& input) noexcept {
    ...
    for (long i = 0; i < N; ++i) {
        for (long j = 0; j < M; ++j) { exps[j] = std::exp(input(i, j)); }
        double sum_exps = 0.0;
        for (auto exp : exps) { sum_exps += exp; }
        const auto inv_sum_exps = 1.0 / sum_exps;
        for (long j = 0; j < M; ++j) { input(i, j) = (exps[j] * inv_sum_exps); }
    }
}

ALTMIN_INLINE auto one_hot_encoding(const nanobind::DRef<Eigen::VectorXd>& input, const int num_classes) noexcept {
    const auto N = input.rows();
    const auto M = num_classes;
    Eigen::MatrixXd res(N, M);
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) { res(i, j) = static_cast<int>(input[i]) == j; }
    }
    return res;
}
...
ALTMIN_INLINE double negative_log_likelihood(const nanobind::DRef<Eigen::MatrixXd>& log_likelihoods,
    ...
    double sum = 0.0;
    for (int i = 0; i < log_likelihoods.rows(); i++) { sum += log_likelihoods(i, targets(i)); }
...
ALTMIN_INLINE auto differentiate_ReLU(const nanobind::DRef<Eigen::MatrixXd>& x) noexcept {
    ...
    Eigen::MatrixXd res(N, M);
    for (long i = 0; i < N; ++i) {
        for (long j = 0; j < M; ++j) { res(i, j) = x(i, j) >= 0.0; }
    }
    return res;
}
